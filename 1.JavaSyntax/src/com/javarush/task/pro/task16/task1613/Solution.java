...
public class Solution {
    ...
    public static boolean isWeekend(LocalDateTime dateTime) {
        return ((dateTime.getDayOfWeek().getValue() >= FRIDAY && (dateTime.getHour()+(dateTime.getDayOfWeek().getValue()*24)) >= WEEKEND_START_FRIDAY_CUT_OFF_HOUR+(FRIDAY*24)) && (dateTime.getDayOfWeek().getValue() <= SUNDAY && dateTime.getHour()+(dateTime.getDayOfWeek().getValue()*24) < WEEKEND_END_SUNDAY_CUT_OFF_HOUR+(SUNDAY*24)));
